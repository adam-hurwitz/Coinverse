...
sealed class ContentResult {
    ...
    data class ContentSelected(val position: Int, var content: Content, var response: String?)
    data class ContentToPlay(var position: Int, var content: Content, var response: String?) : Parcelable {
        ...
        companion object CREATOR : Parcelable.Creator<ContentToPlay> {
            override fun createFromParcel(parcel: Parcel): ContentToPlay { return ContentToPlay(parcel) }
            override fun newArray(size: Int): Array<ContentToPlay?> { return arrayOfNulls(size) }
        }
    }
